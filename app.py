...
st.set_page_config(page_title="Gold import streamlit as st
import yfinance as yf
import pandas as pd
from datetime import datetime
import pytz
...
  GOLD MASTER // TRADING TERMINAL v2.0
</div>
</div>
“””, unsafe_allow_html=True)
...
“””
Returns (bias, entry, tp, sl, rr) based on price location.
Uses ATR-like buffer from the S2-R2 range for SL/TP spacing.
“””
full_range = res2 - sup2
buffer = full_range * 0.015  # 1.5% of range as buffer

```
if price > pivot:
    # Bullish -- buy pullback to R1 or pivot
    bias = "LONG"
    entry = round(max(pivot, res1 - buffer), 2)
    sl = round(entry - (entry - sup1) * 0.5, 2)
    tp = round(res2 - buffer, 2)
else:
    # Bearish -- sell bounce to S1 or pivot
    bias = "SHORT"
...
with col_a:
    st.markdown(
        ...
        + chg_sign + f"{chg:,.2f}" + " (" + chg_sign + f"{chg_pct:.2f}" + "%)"
        + "</div>"
...
with col_b:
    st.markdown(
        ...
        "<span class='level-val' style='color:#ff3355'>" + f"{res2:,.2f}" + "</span>"
        "</div>"
        "<div class='level-row'>"
        "<span class='level-label'>RESISTANCE R1</span>"
        "<span class='level-val' style='color:#ff8080'>" + f"{res1:,.2f}" + "</span>"
        "</div>"
        "<div class='level-row' style='background:rgba(74,158,255,0.04);border-radius:3px;padding:6px 4px;'>"
        "<span class='level-label' style='color:#4a9eff'>PIVOT POINT</span>"
        "<span class='level-val' style='color:#4a9eff'>" + f"{pivot:,.2f}" + "</span>"
        "</div>"
        "<div class='level-row'>"
        "<span class='level-label'>SUPPORT S1</span>"
        "<span class='level-val' style='color:#80ffb0'>" + f"{sup1:,.2f}" + "</span>"
        "</div>"
        "<div class='level-row'>"
        "<span class='level-label'>MAJOR SUPPORT S2</span>"
        "<span class='level-val' style='color:#00ff88'>" + f"{sup2:,.2f}" + "</span>"
        "</div>"
        "</div>",
        unsafe_allow_html=True
    )
...
st.markdown(
    ...
    "<div class='trade-cell-val' style='color:" + ("#00ff88" if bias == "LONG" else "#ff3355") + ";font-size:1.4rem;font-weight:700;letter-spacing:3px'>" + bias + "</div>"
    "</div>"

    "<div class='trade-cell'>"
    "<div class='trade-cell-label'>ENTRY ZONE</div>"
    "<div class='trade-cell-val entry-color'>" + f"{entry:,.2f}" + "</div>"
    "</div>"

    "<div class='trade-cell'>"
    "<div class='trade-cell-label'>TAKE PROFIT</div>"
    "<div class='trade-cell-val tp-color'>" + f"{tp:,.2f}" + "</div>"
    "</div>"

    "<div class='trade-cell'>"
    "<div class='trade-cell-label'>STOP LOSS</div>"
    "<div class='trade-cell-val sl-color'>" + f"{sl:,.2f}" + "</div>"
    "</div>"

    "<div class='trade-cell'>"
    "<div class='trade-cell-label'>RISK : REWARD</div>"
    "<div class='trade-cell-val rr-color'>1 : " + f"{rr}" + "</div>"
    "</div>"

    "<div class='trade-cell'>"
    "<div class='trade-cell-label'>RISK (" + f"{risk_pct:.1f}" + "%)</div>"
    "<div class='trade-cell-val' style='color:#c8d0e0;font-size:0.85rem'>$" + f"{abs(entry - sl):,.2f}" + "/unit</div>"
    "</div>"
...
if bias_type == "bull":
    if cl > res2:
        ...
        alert_msg = "📊 <b>BULLISH BIAS.</b> " + asset + " holding above pivot <b>$" + f"{pivot:,.2f}" + "</b>. Look for long entries on pullbacks to pivot or R1."
elif bias_type == "bear":
    if cl < sup2:
        alert_msg = "🚨 <b>MAJOR SUPPORT BROKEN.</b> " + asset + " below S2 <b>$" + f"{sup2:,.2f}" + "</b>. Short bias active -- significant downside risk."
    elif cl < sup1:
        alert_msg = "🔴 <b>APPROACHING S2.</b> Price below S1 -- bears in control. Short entry near <b>$" + f"{entry:,.2f}" + "</b>, target S2."
    else:
        alert_msg = "⚠️ <b>BEARISH BIAS.</b> " + asset + " below pivot <b>$" + f"{pivot:,.2f}" + "</b>. Look for short entries on bounces to pivot or S1."
else:
    alert_msg = "⏸ <b>RANGE BOUND.</b> " + asset + " is consolidating between S1 and R1. Wait for a clean break above <b>$" + f"{res1:,.2f}" + "</b> or below <b>$" + f"{sup1:,.2f}" + "</b> before entering."

st.markdown(
    "<div class='card'>"
    "<div class='card-label'>MARKET CONTEXT</div>"
    "<div class='alert-box " + alert_class + "'>" + alert_msg + "</div>"
    "</div>",
    unsafe_allow_html=True
)
```
...
reason = “No data returned. Check the symbol or your connection.” if data.empty else “Only “ + str(len(data)) + “ candles – need at least 5.”
st.error(“OFFLINE // “ + reason)
st.info(“Try refreshing or changing the symbol/timeframe.”)", layout="wide", initial_sidebar_state="collapsed")
