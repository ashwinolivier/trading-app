...
if not data.empty and len(data) >= 5:
    ...
    if bias_type == "bull":
        ...
        if cl < sup2:
            alert_msg = "🚨 <b>MAJOR SUPPORT BROKEN.</b> " + asset + " below S2 <b>$" + f"{sup2:,.2f}" + "</b>. Short bias active."
        elif cl < sup1:
            alert_msg = "🔴 <b>APPROACHING S2.</b> Price below S1 -- bears in control. Short entry near <b>$" + f"{entry:,.2f}" + "</b>, target S2."
        else:
            alert_msg = "⚠️ <b>BEARISH BIAS.</b> " + asset + " below pivot <b>$" + f"{pivot:,.2f}" + "</b>. Look for short entries on bounces."
    else:
